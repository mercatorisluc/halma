...
Both are generated by a BFS over chained jumps. A single step goes to an empty
neighbour; a jump hops over an occupied field onto the empty one directly
beyond, and jumps chain.
...
This mutate-and-undo approach is correct today (the reversal is exact — see
invariants) but it is why bot search cannot currently be parallelised.
