...
|---|---|
| `new [seed=N] [agent=PATH] [sampled]` | start a game. Bare: human on seat 1 vs `lookahead2`, as `main.py`. With `agent=models/Talos2.0_round5`: the policy takes **seat 1** and the human seat 2, as `scripts/playAgainstAgent` — the observation is built for seat 1 and `NeuralComputer` refuses any other. |
...
.venv/bin/python .claude/skills/run-halma/driver.py policy models/Talos2.0_round5
# → models/Talos2.0_round5 vs distance: winner=1 in 98 moves
```
